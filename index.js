...
const timestamptotime = (timestamp, func) => {
...
const KeytoDate = Key => {
  return timestamptotime(Key, e => {
    let dayOfWeek = ['日', '月', '火', '水', '木', '金', '土'][e.day]
    let month = formatNumber(e.month)
    let date = formatNumber(e.date)
    let hours = formatNumber(e.hours)
    let minutes = formatNumber(e.minutes)
    let seconds = formatNumber(e.seconds)
    let Milliseconds = e.Milliseconds
    return `${e.year}/${month}/${date}(${dayOfWeek}) ${hours}:${minutes}:${
      seconds
    }.${Milliseconds}`
  })
}
...
app.post('/test/bbs.cgi', function (req, res) {
  ...
  <body>書きこみが終わりました。 [0.0 sec.]<br><br>
      ...
      return l === 3
        ...
        : JCT11280.charAt(
          (c < 160 ? c - 129 : c - 193) * 188 +
              (l === 4
                ? s.charCodeAt(3) - 64
                : (c = parseInt(s.substring(4), 16)) < 127 ? c - 64 : c - 65)
        )
    }
  )
}
